...
import { Loader2, CreditCard, Trash2, Building2 } from 'lucide-react';
...
export function SavedPaymentMethods() {
  ...
  const handleDeleteCard = async (cardId: string) => {
    try {
      const token = localStorage.getItem('ovo-auth-token');
      const response = await fetch(`/api/user/payment-methods/remove-card/${cardId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteAccount = async (accountId: string) => {
    try {
      const token = localStorage.getItem('ovo-auth-token');
      const response = await fetch(`/api/user/payment-methods/remove-account/${accountId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });

      if (response.ok) {
        setSavedAccounts(accounts => accounts.filter(a => a.id !== accountId));
        toast({ title: 'Account Removed', description: 'Bank account deleted successfully' });
      }
    } catch (error) {
  ...
  if (savedCards.length === 0 && savedAccounts.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <CreditCard className="h-12 w-12 mx-auto mb-2 opacity-50" />
        <p>No saved payment methods</p>
        <p className="text-sm">Link a card or bank account in Settings to fund faster</p>
      </div>
    );
  }
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">{card.cardType} {maskCardNumber(card.cardNumber)}</p>
