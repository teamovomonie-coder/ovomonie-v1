...
export async function POST(request: Request) {
  try {
    ...
    if (error || !user?.two_factor_secret || !user?.two_factor_enabled) {
      logger.error('Failed to fetch user 2FA data', { userId, error });
      return NextResponse.json({ message: '2FA not enabled' }, { status: 404 });
    }
    ...
    if (updateError) {
      logger.error('Failed to disable 2FA', { userId, error: updateError });
