  ...
  try {
    ...
    if (res.ok && supabase) {
      ...
      if (pendingPayment?.amount) {
        ...
        logger.info('VFD Authorize OTP: Found pending payment', { amountInKobo, reference });
        ...
        if (userData) {
          ...
          logger.info('VFD Authorize OTP: Balance updated', { newBalance });
          ...
          logger.info('VFD Authorize OTP: Transaction completed', { reference });
        }
      }
    }
    ...
    logger.error('VFD Authorize OTP: Error', { error: message });
