-- Add phone and account columns to notifications table
ALTER TABLE notifications 
ADD COLUMN IF NOT EXISTS sender_phone VARCHAR(20),
ADD COLUMN IF NOT EXISTS sender_account VARCHAR(10),
ADD COLUMN IF NOT EXISTS recipient_phone VARCHAR(20),
ADD COLUMN IF NOT EXISTS recipient_account VARCHAR(10);

-- Update perform_internal_transfer to include all user details in notification
...
BEGIN
  ...
  -- Create notification for sender with ALL details
  INSERT INTO notifications (
    user_id, title, body, category, type, amount, reference,
  ...
  VALUES (
    ...
    p_amount, 
    p_reference,
    v_sender_name, v_sender_phone, v_sender_account,
    v_recipient_name, v_recipient_phone, v_recipient_account
  );
END;
$$ LANGUAGE plpgsql;
